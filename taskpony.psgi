...
sub show_tasks {
    ...
    # Build SQL query
    my $sql = "
        SELECT t.id, t.Title, t.Description, t.AddedDate, t.CompletedDate, p.Title AS ListTitle, p.ListId
        FROM TasksTb t
...
###############################################
        ...
        my $list_deleted = single_db_value("SELECT COUNT(*) FROM ListsTb WHERE id = ? AND DeletedDate IS NOT NULL", $a->{'p.ListId'}) // 0;
print STDERR "DEBUG ($list_deleted) ($a->{'p.ListId'})  \n";        
