...
echo "Fetching latest from local repo and merging dev into main..."
cd /opt/taskpony
git checkout main
git pull origin main
git merge dev

echo "Press any key to continue, or Ctrl+C to abort...  Next step, tag version $ver"
read -r -n 1 -s
...
echo "Press any key to continue, or Ctrl+C to abort...  Next Step, build Docker image"
read -r -n 1 -s
